...
class VRAMManager:
    ...
    Tiers:
        high   (>=14GB): float16, all models on GPU, no swapping
        medium (12-13GB): float16, model swapping between pipeline stages
    ...
    def _resolve_dtype(self, precision: str) -> torch.dtype:
        ...
        else:  # auto — float16 for all tiers (sufficient quality, better perf)
            return torch.float16
